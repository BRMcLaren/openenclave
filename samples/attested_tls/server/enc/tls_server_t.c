    ...
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    setup_tls_server_args_t* pargs_in = (setup_tls_server_args_t*)input_buffer;
    setup_tls_server_args_t* pargs_out = (setup_tls_server_args_t*)output_buffer;
    ...
    if (pargs_in->port)
        OE_SET_IN_POINTER(port, pargs_in->port_len * sizeof(char), char*);
    ...
    OE_CHECK_NULL_TERMINATOR(pargs_in->port, pargs_in->port_len);

    /* lfence after checks. */
    oe_lfence();

    /* Call user function. */
    pargs_out->_retval = setup_tls_server(
        pargs_in->port);
...
oe_ecall_func_t __oe_ecalls_table[] = {
    (oe_ecall_func_t) ecall_setup_tls_server
};
